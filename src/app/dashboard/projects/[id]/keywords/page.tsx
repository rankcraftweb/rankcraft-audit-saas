...
function getIntentClass(intent: string) {
  if (intent === "Commercial") return "border-[#d4af37]/50 bg-[#fff8df] text-[#7a5b00]";
  if (intent === "Brand") return "border-[#2b2413] bg-[#111111] text-[#d4af37]";
...
function getOpportunityScore(keyword: GscKeywordRow) {
  ...
  let score = 0;
  if (position > 3 && position <= 15) score += 45;
  if (position > 15 && position <= 50) score += 25;
  if (impressions >= 50) score += 30;
  else if (impressions >= 10) score += 20;
  else if (impressions > 0) score += 10;
  if (clicks === 0 && impressions > 0) score += 20;
  if (ctr < 0.02 && impressions >= 10) score += 15;
...
function getLatestKeywordRows(rows: GscKeywordRow[]) {
  ...
  if (!range) return [];
  return rows.filter(
    (r) => r.start_date === range.startDate && r.end_date === range.endDate
  );
}
...
function getLowCtrKeyword(rows: GscKeywordRow[]) {
  return rows
    ...
    .sort((a, b) => {
      const diff = Number(a.ctr || 0) - Number(b.ctr || 0);
      if (diff !== 0) return diff;
...
export default async function KeywordsPage({ params }: KeywordsPageProps) {
  ...
  const latestKeywordDate = latestRange
    ? `${formatShortDate(latestRange.startDate)} – ${formatShortDate(latestRange.endDate)}`
    : "No keyword data yet";
  ...
  const averagePosition =
    keywordList.length > 0
      ? keywordList.reduce((sum, k) => sum + Number(k.position || 0), 0) / keywordList.length
      : null;
  ...
  const topOpportunities: KeywordInsight[] = keywordList
    .map((k) => ({
      ...k,
      opportunityScore: getOpportunityScore(k),
      intent: getKeywordIntent(k.query),
      status: getKeywordStatus(k),
      reason: getOpportunityReason(k),
    }))
    .filter((k) => k.opportunityScore > 0)
    .sort((a, b) => b.opportunityScore - a.opportunityScore)
    .slice(0, 5);

  const keywordRows: KeywordInsight[] = keywordList.map((k) => ({
    ...k,
    opportunityScore: getOpportunityScore(k),
    intent: getKeywordIntent(k.query),
    status: getKeywordStatus(k),
    ...
    <div className="space-y-5">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            ...
            <p className="mt-2 text-xs text-slate-300">
              Date range: <span className="font-semibold text-white">{latestKeywordDate}</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <p className="mt-1 text-lg font-bold text-white">{formatCtr(averageCtr)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {keywordList.length === 0 ? (
        <div className="flex min-h-[260px] flex-col items-center justify-center rounded-2xl border border-dashed border-[#d4af37]/40 bg-white p-8 text-center">
          <p className="text-sm font-semibold text-slate-950">No keyword data yet</p>
        ...
        <>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-[#e6dcc8] bg-white p-4">
              <p className="text-xs font-semibold text-slate-950">Best Ranking Keyword</p>
              <p className="mt-2 text-base font-bold text-slate-950">
                {bestRankingKeyword?.query || "No ranking data yet"}
              </p>
              <p className="mt-2 text-xs leading-5 text-slate-500">
                {bestRankingKeyword
                  ? `Position ${formatPosition(bestRankingKeyword.position)} with ${formatNumber(bestRankingKeyword.impressions)} impressions.`
                  : "Ranking wins will appear here once position data is available."}
              </p>
            </div>

            <div className="rounded-2xl border border-[#d4af37]/40 bg-[#fff8df] p-4">
              <p className="text-xs font-semibold text-[#7a5b00]">Top Opportunity</p>
              ...
              <p className="mt-2 text-xs leading-5 text-[#7a5b00]/70">
                {topOpportunityKeyword
                  ? `Position ${formatPosition(topOpportunityKeyword.position)} with ${formatNumber(topOpportunityKeyword.impressions)} impressions.`
                  : "Keywords in positions 4–15 will appear here."}
              </p>
            </div>

            <div className="rounded-2xl border border-[#e6dcc8] bg-[#faf7ef] p-4">
              <p className="text-xs font-semibold text-slate-950">CTR Review</p>
              <p className="mt-2 text-base font-bold text-slate-950">
                {lowCtrKeyword?.query || "No CTR issue yet"}
              </p>
              <p className="mt-2 text-xs leading-5 text-slate-500">
                {lowCtrKeyword
                  ? `${formatCtr(lowCtrKeyword.ctr)} CTR from ${formatNumber(lowCtrKeyword.impressions)} impressions.`
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-[#e6dcc8] bg-white p-4">
              ...
              <p className="mt-1 text-[11px] text-slate-500">Positions 1–10.</p>
            </div>
            <div className="rounded-2xl border border-[#e6dcc8] bg-white p-4">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
                Opportunities
              ...
              <p className="mt-1 text-[11px] text-slate-500">Close to stronger movement.</p>
            </div>
            <div className="rounded-2xl border border-[#e6dcc8] bg-white p-4">
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500">
                Low CTR Keywords
              ...
              <p className="mt-1 text-[11px] text-slate-500">Visibility with weak clicks.</p>
            </div>
          </div>
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          <span
                            className={`rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${getIntentClass(keyword.intent)}`}
                          >
                            {keyword.intent}
                          </span>
                          <span
                            className={`rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${getKeywordStatusClass(keyword.status)}`}
                          >
                            {keyword.status}
                          </span>
                      ...
                      <p className="mt-3 text-xs leading-5 text-slate-500">{keyword.reason}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="rounded-2xl border border-[#e6dcc8] bg-white">
            <div className="border-b border-[#eee5d4] px-5 py-4">
              ...
              <p className="mt-0.5 text-sm font-bold text-slate-950">All keywords</p>
            </div>
                  ...
                  {keywordRows.map((keyword) => (
                    <tr key={keyword.id} className="hover:bg-[#faf7ef]">
                      ...
                      <td className="px-3 py-2.5">
                        <span
                          className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-medium ${getIntentClass(keyword.intent)}`}
                        >
                          {keyword.intent}
                        </span>
                      </td>
                      <td className="px-3 py-2.5">
                        <span
                          className={`inline-flex min-w-[100px] justify-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${getKeywordStatusClass(keyword.status)}`}
                        >
                          {keyword.status}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-2.5 text-slate-500">
                        {formatShortDate(keyword.start_date)} – {formatShortDate(keyword.end_date)}
                      </td>
                    </tr>
                  ))}
                </tbody>
